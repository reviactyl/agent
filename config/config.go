...
type SystemConfiguration struct {
	...
	Data string `default:"/var/lib/reviactyl/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/reviactyl/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/reviactyl/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Reviactyl installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/reviactyl" json:"-" yaml:"tmp_directory"`
...
type Configuration struct {
	...
	AppName string `default:"Reviactyl" json:"app_name" yaml:"app_name"`
...
// configuration update cannot leave Agent and the Panel using different keys.
func (c *Configuration) ResolveToken(remote bool) error {
	...
	var err error
	if c.Token.ID, err = resolve("token ID", os.Getenv("AGENT_TOKEN_ID"), c.AuthenticationTokenId); err != nil {
		return err
	}
	if c.Token.Token, err = resolve("token", os.Getenv("AGENT_TOKEN"), c.AuthenticationToken); err != nil {
		return err
	}
	return nil
}
...
// EnsureReviactylUser ensures that the Reviactyl core user exists on the
...
func EnsureReviactylUser() error {
	...
	log.WithField("username", _config.System.Username).Info("checking for reviactyl system user")
